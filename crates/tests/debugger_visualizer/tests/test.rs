...
pstr             : "Hello World!" [Type: windows::core::strings::pstr::PSTR]
...
pcstr            : "Hello World!" [Type: windows::core::strings::pcstr::PCSTR]
...
pwstr            : "Hello World!" [Type: windows::core::strings::pwstr::PWSTR]
    [<Raw View>]     [Type: windows::core::strings::pwstr::PWSTR]
    [len]            : 0xc
    [chars]
        [0]              : 0x48 'H' [Type: char16_t]
        [1]              : 0x65 'e' [Type: char16_t]
        [2]              : 0x6c 'l' [Type: char16_t]
        [3]              : 0x6c 'l' [Type: char16_t]
        [4]              : 0x6f 'o' [Type: char16_t]
        [5]              : 0x20 ' ' [Type: char16_t]
        [6]              : 0x57 'W' [Type: char16_t]
        [7]              : 0x6f 'o' [Type: char16_t]
        [8]              : 0x72 'r' [Type: char16_t]
        [9]              : 0x6c 'l' [Type: char16_t]
        [10]             : 0x64 'd' [Type: char16_t]
        [11]             : 0x21 '!' [Type: char16_t]

pcwstr           : "Hello World!" [Type: windows::core::strings::pcwstr::PCWSTR]
...
fn test_debugger_visualizer() {
    let mut string = "Hello World!\0".to_string();
    ...
    let pstr = PSTR::from_raw(string.as_mut_ptr());
    unsafe {
        assert_eq!(&string.as_bytes()[..last_index], pstr.as_bytes());
    }

    // Test debugger visualizations for PCSTR
    let pcstr = PCSTR::from_raw(string.as_ptr());
    unsafe {
        assert_eq!(&string.as_bytes()[..last_index], pcstr.as_bytes());
    }

    // Test debugger visualizations for PWSTR
    let mut s: Vec<u16> = vec![72, 101, 108, 108, 111, 32, 87, 111, 114, 108, 100, 33, 0];
    let pwstr = PWSTR::from_raw(s.as_mut_ptr());
    unsafe {
        assert_eq!(&s.as_slice()[..last_index], pwstr.as_wide());
    }

    // Test debugger visualizations for PCWSTR
    let pcwstr = PCWSTR::from_raw(s.as_ptr());
    unsafe {
        assert_eq!(&s.as_slice()[..last_index], pcwstr.as_wide());
    }
